...
app.post('/api/add-device', (req, res) => {
  const { name, description, device_type_id, user_id } = req.body;
  ...
  //  Sinh API key ngẫu nhiên 32 bytes (64 ký tự hex)
  const api_key = crypto.randomBytes(32).toString('hex');

  const sql = `
    INSERT INTO devices (name, device_id, description, device_type_id, api_key, user_id, createAt, updateAt)
    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
  `;
...
app.get("/api/filter", async (req, res) => {
  ...
  try {
    ...
    // ============================
    //  CHẾ ĐỘ 1 — LỌC THEO DEVICE
    // ============================
    if (device_id && !sensor && !start && !end) {
      result.mode = "device";
      const prefix = `device_id=${device_id}/`;
      ...
      for await (const obj of stream) {
        ...
        if (!result.sensors[rec.sensor]) result.sensors[rec.sensor] = [];
        result.sensors[rec.sensor].push(rec);
    ...
    if (!device_id && sensor && !start && !end) {
      ...
      for await (const obj of stream) {
        if (!obj.name.includes(`sensor=${sensor}`)) continue;
        const rec = await readJSON(obj.name);
        if (!rec) continue;

        if (!result.devices[rec.device_id]) result.devices[rec.device_id] = [];
        result.devices[rec.device_id].push(rec);
    ...
    // ===============================
    //  CHẾ ĐỘ 3 — LỌC THEO THỜI GIAN
    //  (có thể kèm sensor hoặc device)
    // ===============================
    result.mode = "time";
...
app.get("/api/dataset", async (req, res) => {
  ...
  try {
    ...
    // ========================================================
    // 1️⃣ CHẾ ĐỘ: DATASET THEO THIẾT BỊ
    // ========================================================
    if (device_id && !sensor) {
      dataset.mode = "device";
      ...
      for await (const obj of stream) {
        ...
        // gom theo sensor
        if (!dataset.sensors[rec.sensor]) dataset.sensors[rec.sensor] = [];
        dataset.sensors[rec.sensor].push(rec);
    ...
    // ========================================================
    // 2️⃣ CHẾ ĐỘ: DATASET THEO SENSOR (GLOBAL)
    // ========================================================
    if (sensor && !device_id) {
      dataset.mode = "sensor";
      const stream = minioClient.listObjectsV2("thingsboard-data", "", true);
      ...
      for await (const obj of stream) {
        ...
        // gom theo thiết bị
        if (!dataset.devices[rec.device_id]) dataset.devices[rec.device_id] = [];
        dataset.devices[rec.device_id].push(rec);
    ...
    // ========================================================
    // 3️⃣ CHẾ ĐỘ: DATASET THEO THỜI GIAN + KẾT HỢP
    // ========================================================
    dataset.mode = "time";
